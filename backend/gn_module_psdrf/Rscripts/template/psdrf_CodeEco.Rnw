...
t.m <- t
t.m$Note <- CodeEcologie$Descriptif[match(t.m$CodeEcolo, paste(tolower(CodeEcologie$Codification), CodeEcologie$Code))]
t.m <- 
  t.m %>%
  group_by(NumDisp, Disp, Cycle, Note, NbrePlacettes) %>%
  summarise(Nha = sum(Nha, na.rm = T)) %>%
...
t3 <-
  ...
  # le left_join coerce Cat en character → re-factoriser pour préserver l'ordre dendrométrique
  mutate(Cat = factor(Cat, levels = c("PER", "PB", "BM", "GB", "TGB"))) %>%
  arrange(Note)
