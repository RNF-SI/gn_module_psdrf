...
safe_process_data <- function(data) {
  # Si les données sont NULL ou vides, retourner un data frame vide
  ...
  # Ajouter une vérification supplémentaire pour les valeurs négatives
  data$Vha[data$Vha < 0] <- NA_real_
  
  # S'assurer que le filtrage !is.na(Vha) & Vha != 0 fonctionnera
  data <- data %>%
    mutate(
      Vha = as.numeric(Vha),
      Vha = ifelse(is.na(Vha), NA_real_, Vha)
    )
...
get_radar_coords <- function(group = NULL, tables_needed = NULL) { # à tester par habitat
 # group <- "Habitat" # debug
  ...
  # arbres sur pied
  vol_bv <- 
    safe_process_data(tables_needed$grouped_results_of_Fpied_by_) %>% 
  ...
  # taillis
  vol_taillis <- 
    safe_process_data(tables_needed$grouped_results_of_Taillis_by_) %>% 
    filter(!is.na(Vha) & Vha != 0) %>% 
    select(NumDisp, group, Cycle, Vha) %>% 
  ...
  # bmp
  vol_bmp <- 
    safe_process_data(tables_needed$grouped_results_of_BMP_by_) %>% 
    filter(!is.na(Vha) & Vha != 0) %>% 
    select(NumDisp, group, Cycle, Vha) %>% 
    rename(VhaBMP = Vha)
      ...
      # bms
  vol_bms <- 
    safe_process_data(tables_needed$grouped_results_of_BMS_by_) %>% 
    filter(!is.na(Vha) & Vha != 0) %>% 
    select(NumDisp, group, Cycle, Vha) %>% 
    rename(VhaBMS = Vha)
