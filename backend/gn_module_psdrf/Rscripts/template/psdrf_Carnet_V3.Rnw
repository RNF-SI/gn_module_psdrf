...
tryCatch({
  ...
  if (!inherits(ser, c("SpatialPointsDataFrame", "SpatialPolygonsDataFrame"))) {
    cat("L'objet 'ser' n'est pas de type spatial après conversion, création d'un objet par défaut\n")
    empty_df <- data.frame(NomSER = "Dummy")
    coordinates <- matrix(c(0, 0), ncol=2)
    sp <- SpatialPoints(coordinates)
    ser <- SpatialPointsDataFrame(sp, empty_df)
  ...
  tryCatch({
    if (nrow(ser@data) == 0) {
      cat("L'objet 'ser' n'a pas de lignes de données, ajout d'une ligne par défaut\n")
      empty_df <- data.frame(NomSER = "Dummy")
      coordinates <- matrix(c(0, 0), ncol=2)
      sp <- SpatialPoints(coordinates)
      ser <- SpatialPointsDataFrame(sp, empty_df)
...
process_src_final <- function() {
  ...
  if(!exists("src_Final") || is.null(src_Final)) {
    return("\\textbf{Erreur: Variable src\\_Final manquante ou NULL}")
  }
  
  safe_src <- unlist(src_Final)
  if(length(safe_src) == 0) {
    return("\\textbf{Erreur: src\\_Final est vide après unlist()}")
  }
  
  result <- tryCatch({
    suppressWarnings(knit_child(text = safe_src))
  }, error = function(e) {
    paste0("\\textbf{Erreur lors de la génération: ", gsub("_", "\\\\_", e$message), "}")
  })
...
process_src_evolfinal <- function() {
  ...
  if(!exists("src_EvolFinal") || is.null(src_EvolFinal)) {
    return("\\textbf{Erreur: Variable src\\_EvolFinal manquante ou NULL}")
  }
  
  safe_src <- unlist(src_EvolFinal)
  if(length(safe_src) == 0) {
    return("\\textbf{Erreur: src\\_EvolFinal est vide après unlist()}")
  }
  
  result <- tryCatch({
    suppressWarnings(knit_child(text = safe_src))
  }, error = function(e) {
    paste0("\\textbf{Erreur lors de la génération de l'évolution: ", gsub("_", "\\\\_", e$message), "}")
  })
