...
# Traitement sécurisé des données de franc-pied
tab <- tryCatch({
  temp_data <- safe_process_data(grouped_results_of_Fpied_by_Classe)
  # Utiliser une approche alternative au filtre direct
  valid_rows <- !is.na(temp_data$Classe)
  temp_data <- temp_data[valid_rows, ]
  # Remplacer NA par 0 de manière sécurisée
  temp_data$Vha <- ifelse(is.na(temp_data$Vha), 0, temp_data$Vha)
  temp_data$Population <- "Tiges de\nfranc-pied"
  temp_data
}, error = function(e) {
  warning("Erreur lors du traitement des données de franc-pied: ", e$message)
  data.frame(
    Classe = numeric(0),
    Vha = numeric(0),
...
# Traitement sécurisé des données de taillis
tab2 <- tryCatch({
  # Approche sans utiliser %>% et filter
  temp_data <- safe_process_data(grouped_results_of_Taillis_by_Classe)
  # Filtrage explicite
  valid_rows <- !is.na(temp_data$Classe)
  temp_data <- temp_data[valid_rows, ]
  # Remplacement sécurisé des NA
  temp_data$Vha <- ifelse(is.na(temp_data$Vha), 0, temp_data$Vha)
  temp_data
}, error = function(e) {
  warning("Erreur lors du traitement des données de taillis: ", e$message)
  data.frame(
    Classe = numeric(0),
    Vha = numeric(0)
  )
...
tab <- tryCatch({
  tab %>% 
    rbind(tab2) %>%
    mutate(
      Titre = paste0("Cycle ", Cycle),
      Vha = as.numeric(Vha)
    )
}, error = function(e) {
  warning("Erreur lors de la combinaison des données : ", e$message)
  data.frame(
    Classe = numeric(0),
    Vha = numeric(0),
    Population = character(0),
...
tab3 <- tryCatch({
  tab %>% 
    ...
    summarise(som = round(sum(Vha, na.rm = TRUE), 0)) %>%
    ungroup()
}, error = function(e) {
  warning("Erreur lors du calcul des sommes : ", e$message)
  data.frame(
    Cycle = numeric(0),
    Titre = character(0),
    som = numeric(0)
...
X <- ifelse(length(tab$Classe) > 0, max(tab$Classe, na.rm = TRUE) - 20, 0)
Y <- ifelse(length(tab$Vha) > 0, max(tab$Vha, na.rm = TRUE) - 10, 0)
...
if (length(pos) > 0) {
  ...
  t3b <- as.data.frame(t3b)

  t3b[pos2, "Cycle"] <- paste0(
    "\\multirow{", t3b$Count[pos2], 
    "}{*}{\\parbox{0.7cm}{\\centering ", 
    t3b$Cycle[pos2], "}}"

  )
  t3b[pos2, "NbrePlacettes"] <- paste0(
    "\\multirow{", t3b$Count[pos2], 
...
df2 <- 
  ...
  summarize(Nha = sum(Nha)) %>% 
  ungroup() %>% 
  mutate(Cycle = paste0("Cycle ", Cycle))
