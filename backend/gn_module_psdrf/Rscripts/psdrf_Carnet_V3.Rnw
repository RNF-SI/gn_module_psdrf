...
#repPSDRF <- "/Users/Valentin/Travail/Outils/GitHub/PermPSDRF2"
...
repSav <- paste0(repPSDRF, "/out/", disp_num, "-", Name_rep)

# ----- définition du répertoire de sauvegarde du livret et des figures
repPdf <- paste0(
  repPSDRF, "/out/", disp_num, "-", Name_rep, "/carnet/"
)
repFigures <- paste0(repPdf, "figures")
...
load(file.path(repSav, "tables/psdrfTablesBrutes.Rdata"))
load(file.path(repSav, "tables/psdrfTablesElaboreesPlac.Rdata"))
for(i in 1:length(TabPla)) {assign(names(TabPla)[i], TabPla[[i]])}
load(file.path(repSav, "tables/psdrfTablesElaborees.Rdata"))
